...
tbl_summary(
  ...
  include = c(
    ...
    eyesight_cat, glasses, age_bir, sleep_wkdy, sleep_wknd
  )
)
...
tbl_summary(
  ...
  include = c(
    ...
    eyesight_cat, glasses, age_bir, sleep_wkdy, sleep_wknd, income
  ),
  label = list(
    race_eth_cat ~ "Race/ethnicity",
    eyesight_cat ~ "Eyesight",
...
tbl_summary(nlsy,
						...
						statistic= list(
							sleep_wkdy~ "{mean}, {min}-{max}",
							sleep_wknd~ "{mean}, {min}-{max}",
							income~ "{p10} - {p90}"
						),
